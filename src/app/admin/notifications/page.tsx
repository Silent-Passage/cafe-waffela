...
import { Plus, Trash2, Loader2, MessageSquare, ShoppingBag } from "lucide-react";
...
export default function NotificationsPage() {
  ...
  const create = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["notifications"] }); setOpen(false); },
  });
  ...
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); create.mutate(form); };
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <button onClick={() => { setForm({ type: "order", message: "" }); setOpen(true); }}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {notifications?.map((n) => (
            <div key={n.id} className="bg-card rounded-2xl shadow-sm border border-border p-6 relative group hover:shadow-md transition-all">
              <button onClick={() => { if (confirm("Delete?")) remove.mutate(n.id); }}
              ...
              <div className="flex items-center gap-3 mb-3">
                <div className={`p-2 rounded-lg ${n.type === "order" ? "bg-amber-100 text-amber-600" : "bg-blue-100 text-blue-600"}`}>
                  {n.type === "order" ? <ShoppingBag className="w-4 h-4" /> : <MessageSquare className="w-4 h-4" />}
                </div>
                <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{n.type}</span>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[{ val: "order", Icon: ShoppingBag }, { val: "review", Icon: MessageSquare }].map(({ val, Icon }) => (
                    <label key={val} className={`border-2 rounded-xl p-3 flex flex-col items-center gap-2 cursor-pointer transition-colors ${form.type === val ? "border-primary bg-primary/5 text-primary" : "border-border hover:bg-secondary/50 text-muted-foreground"}`}>
                      <input type="radio" name="type" value={val} className="sr-only" checked={form.type === val} onChange={() => setForm({ ...form, type: val })} />
                      <Icon className="w-6 h-6" />
                      <span className="font-semibold text-sm capitalize">{val}</span>
                    </label>
                  ))}
                </div>
              </div>
              <div>
                ...
                <textarea required rows={3} placeholder="e.g. 🧇 Someone just ordered a Waffle!" value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })}
                ...
                <p className="text-xs text-muted-foreground mt-1">Emojis highly recommended!</p>
              </div>
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" disabled={create.isPending} className="px-6 py-2 text-sm font-bold text-primary-foreground bg-primary rounded-xl shadow-md hover:bg-primary/90 disabled:opacity-50 flex items-center gap-2">
